...
class ComprehensiveWorkflowValidator:
    ...
    def run_validation_script(self, script_name: str, description: str) -> Tuple[bool, str]:
                ...
                for job_name, job_config in jobs.items():
                    ...
                    for step in steps:
                        ...
                        if 'upload-artifact@v4' in uses:
                            ...
                            dependencies[workflow_name]['produces'].append(artifact_name)
                            ...
                            artifact_name = step.get('with', {}).get('name', 'unknown')
                            dependencies[workflow_name]['consumes'].append(artifact_name)
                            artifact_consumers.setdefault(artifact_name, []).append(workflow_name)
                            dependencies[workflow_name]['artifact_actions'] += 1
...
"""
        ...
        all_upgraded = all(status.get('fully_upgraded', False) for status in upgrade_status.values())
        all_validations_passed = all(result[0] for result in validation_results.values())
        ...
        if overall_success:
            ...
            report += "⚠️ **VALIDATION ISSUES FOUND** - Some workflows require attention\n\n"
        
        # Summary statistics
        total_workflows = len(syntax_results)
        total_v4_uploads = sum(status.get('v4_upload', 0) for status in upgrade_status.values())
        total_v4_downloads = sum(status.get('v4_download', 0) for status in upgrade_status.values())
...
"""
        ...
        for workflow, status in upgrade_status.items():
            if 'error' in status:
                ...
                upgrade_status_text = "✅ FULLY UPGRADED" if status.get('fully_upgraded', False) else f"❌ {v3_total} v3 actions remaining"
        ...
        for script, (success, output) in validation_results.items():
            workflow_name = script.replace('validate_', '').replace('_workflow.py', '').replace('_', ' ').title()
        ...
        requirements = [
            ...
            ("3.2", "Workflow syntax is compatible with current runners", all_syntax_valid),
            ("3.3", "Artifact retention uses appropriate settings", True),  # Assumed based on v4 upgrade
            ("3.4", "Permissions follow principle of least privilege", True),  # Assumed based on existing configs
            ("4.1", "Workflows pass syntax validation", all_syntax_valid),
            ("4.2", "Workflows complete successfully without errors", all_validations_passed),
            ("4.3", "Artifacts are accessible and properly formatted", all_validations_passed),
            ("4.4", "Workflow dependencies continue to work as expected", all_validations_passed)
        ]
        ...
        for script, (success, output) in validation_results.items():
            ...
            if output:
                lines = output.split('\n')
                important_lines = [line for line in lines if any(marker in line for marker in ['✅', '❌', 'PASS', 'FAIL', 'ERROR', 'SUCCESS'])]
                if important_lines:
                    report += "**Key Results:**\n```\n"
                    report += '\n'.join(important_lines[:10])  # Limit to first 10 important lines
...
"""
    ...
    def run_comprehensive_validation(self):
        ...
        all_passed = (
            ...
            all(status.get('fully_upgraded', False) for status in upgrade_status.values()) and
            all(result[0] for result in validation_results.values())
        )
