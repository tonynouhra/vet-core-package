...
class UpgradeTestingPipeline:
    ...
    def test_upgrade_single_version(
        ...
        logger.info(
            f"Testing upgrade {package_name} -> {target_version} with {python_exe}"
        )
...
"""
    ...
    def test_upgrade_multi_version(
        ...
        """
        logger.info(
            f"Testing upgrade {package_name} -> {target_version} across Python versions"
        )
            ...
            for future in concurrent.futures.as_completed(future_to_version):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        logger.error(
                            f"❌ Upgrade failed for Python {version}: {result.error_message}"
                        )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if vuln_still_present:
                        logger.warning(
                            f"❌ Vulnerability {vuln.id} still present after upgrade"
                        )
                    else:
    ...
    def run_comprehensive_upgrade_test(
        ...
        """
        logger.info(
            f"Running comprehensive upgrade test: {package_name} -> {target_version}"
        )
        ...
        try:
            ...
            multi_version_results = self.test_upgrade_multi_version(
                package_name, target_version
            )
            results["multi_version_results"] = {
                version: asdict(result)
                for version, result in multi_version_results.items()
            }

            # Check if all versions passed
            all_versions_passed = all(
                result.success for result in multi_version_results.values()
            )
            ...
            current_metrics, regressions = self.test_performance_impact(
                package_name, target_version
            )
            ...
            results["performance_results"] = {
                ...
                "has_significant_regressions": any(
                    reg.is_significant for reg in regressions
                ),
            }
            ...
            has_significant_regressions = results["performance_results"][
                "has_significant_regressions"
            ]

            results["overall_success"] = (
                all_versions_passed
                and all_fixes_verified
                and not has_significant_regressions
            )
    ...
    def run_vulnerability_upgrade_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pipeline_results["vulnerabilities_found"] = [
    ...
    def generate_pipeline_report(
        ...
        for package_name, test_result in upgrade_tests.items():
            ...
            if perf_results:
                ...
                if regressions:
                    ...
                    report_lines.append(
                        "**Performance:** No significant regressions detected"
                    )
                report_lines.append("")
        ...
        if overall_success:
            report_lines.append(
                "All upgrade tests passed successfully. Dependencies can be safely upgraded."
            )
        else:
            report_lines.extend(
                [
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path.cwd(), help="Directory to save reports"
    )

    parser.add_argument(
        "--baseline-file", type=Path, help="Path to performance baseline file"
    )

    parser.add_argument(
        "--package", help="Test specific package upgrade (format: package==version)"
    )
    ...
    try:
        if args.package:
            ...
            logger.info(
                f"Testing specific package upgrade: {package_name} -> {target_version}"
            )
            ...
            # Save results
            results_file = (
                args.output_dir / f"upgrade_test_{package_name}_{target_version}.json"
            )
            ...
            # Save results
            results_file = (
                args.output_dir / "vulnerability_upgrade_pipeline_results.json"
            )
            with open(results_file, "w") as f:
                json.dump(results, f, indent=2)
