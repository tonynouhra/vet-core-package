...
def create_config(
    ...
    try:
        if interactive:
            ...
            # Get basic settings
            scan_schedule = (
                input("Scan schedule (cron format) [0 6 * * *]: ").strip()
                or "0 6 * * *"
            )

            # Auto-fix settings
            auto_fix_enabled = (
                input("Enable auto-fix? (y/n) [n]: ").strip().lower() == "y"
            )

            if auto_fix_enabled:
                print("Available severity levels: critical, high, medium, low")
                max_severity = (
                    input("Maximum auto-fix severity [medium]: ").strip() or "medium"
                )
                try:
...
def show_config(
    ...
    try:
        ...
        if format_type == "json":
            ...
            print("Notifications:")
            print(
                f"  Email: {', '.join(config.notifications.email) if config.notifications.email else 'None'}"
            )
            print(
        ...
        if len(parts) == 2:
            ...
            if section == "auto_fix":
                if key == "enabled":
                    ...
                    config.auto_fix.require_approval = value.lower() in (
                        "true",
                        "yes",
                        "1",
                    )
...
def main():
    """Main CLI function."""
    parser = argparse.ArgumentParser(description="Manage security configurations")
    parser.add_argument(
        "--config-dir",
        "-c",
        type=Path,
    ...
    try:
        if args.command == "create":
            create_config(
                config_manager, Environment(args.environment), args.interactive
            )
        elif args.command == "show":
            show_config(config_manager, Environment(args.environment), args.format)
        elif args.command == "list":
            list_environments(config_manager)
        elif args.command == "update":
            update_config(
                config_manager, Environment(args.environment), args.setting, args.value
            )
