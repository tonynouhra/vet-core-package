            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                reports = {
                    ...
                    "monthly-safety-report.json": {
                        ...
                        "remediations": {"recommended": [], "other": []},
                    },
                    "monthly-pip-audit-report.json": {
                        "vulnerabilities": [],
                        "summary": {"total": 0, "unique": 0},
                        "dependencies": 25,
                        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                    },
                }
                ...
                for filename in expected_files:
                    if not (temp_path / filename).exists():
                        self.log(
                            f"❌ Missing security report file: {filename}", "ERROR"
                        )
                        return False
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for filename in expected_files:
                    if not (temp_path / filename).exists():
                        self.log(
                            f"❌ Missing dependency report file: {filename}", "ERROR"
                        )
                        return False
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                reports = {
                    ...
                    "monthly-maintainability-report.json": {
                        "src/vet_core/models/base.py": {"mi": 85.2, "rank": "A"},
                        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                    },
                ...
                for filename in expected_files:
                    if not (temp_path / filename).exists():
                        self.log(
                            f"❌ Missing code quality report file: {filename}", "ERROR"
                        )
                        return False
