...
from sqlalchemy import (
    Boolean, CheckConstraint, Enum, Index, String, Text, UniqueConstraint
)
...
class Clinic(BaseModel):
    ...
    address_line2: Mapped[Optional[str]] = mapped_column(
        String(255),
        nullable=True,
        comment="Secondary address line (suite, unit, etc.)"
    )
    ...
    latitude: Mapped[Optional[float]] = mapped_column(
        nullable=True,
        comment="Latitude coordinate for location-based searches"
    )
    
    longitude: Mapped[Optional[float]] = mapped_column(
        nullable=True,
        comment="Longitude coordinate for location-based searches"
    )
    ...
    operating_hours: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Operating hours by day of week stored as JSON"
    )
    ...
    services_offered: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of services offered by the clinic"
    )
    
    specialties: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of medical specialties available"
    )
    ...
    description: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Description of the clinic and its services"
    )
    ...
    equipment_available: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of medical equipment available"
    )
    ...
    number_of_exam_rooms: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Number of examination rooms"
    )
    
    number_of_surgery_rooms: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Number of surgery rooms"
    )
    ...
    insurance_accepted: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of accepted insurance providers"
    )
    
    payment_methods: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of accepted payment methods"
    )
    ...
    after_hours_instructions: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Instructions for after-hours emergencies"
    )
    ...
    photos: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Array of photo URLs for the clinic"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_clinics_operating_hours_gin', 'operating_hours', postgresql_using='gin'),
        Index('idx_clinics_services_offered_gin', 'services_offered', postgresql_using='gin'),
        Index('idx_clinics_specialties_gin', 'specialties', postgresql_using='gin'),
        Index('idx_clinics_facility_features_gin', 'facility_features', postgresql_using='gin'),
        Index('idx_clinics_equipment_gin', 'equipment_available', postgresql_using='gin'),
        Index('idx_clinics_insurance_gin', 'insurance_accepted', postgresql_using='gin'),
        Index('idx_clinics_payment_methods_gin', 'payment_methods', postgresql_using='gin'),
        
        # Full-text search indexes
        Index('idx_clinics_name_search', 'name', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'}),
        Index('idx_clinics_description_search', 'description', postgresql_using='gin', postgresql_ops={'description': 'gin_trgm_ops'}),
    )
    ...
    def set_operating_hours(self, day_of_week: str, open_time: str, close_time: str,
                           is_open: bool = True, lunch_break: Optional[Dict[str, str]] = None) -> None:
        """
    ...
    def accepts_insurance(self, insurance_provider: str) -> bool:
        ...
        """
        return (
            self.insurance_accepted is not None and
            insurance_provider in self.insurance_accepted
        )
    ...
    def accepts_payment_method(self, payment_method: str) -> bool:
        ...
        """
        return (
            self.payment_methods is not None and
            payment_method in self.payment_methods
        )
    
    def update_status(self, new_status: ClinicStatus, reason: Optional[str] = None) -> None:
        """
    ...
    def is_within_radius(self, latitude: float, longitude: float, radius_km: float) -> bool:
        """
