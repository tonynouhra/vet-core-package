...
class Appointment(BaseModel):
    ...
    reason: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Reason for the appointment or chief complaint"
    )

    notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Additional notes about the appointment"
    )

    internal_notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Internal staff notes not visible to pet owner"
    )
    ...
    cancellation_reason: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True, comment="Reason for appointment cancellation"
    )

    # Follow-up information
    follow_up_needed: Mapped[Optional[bool]] = mapped_column(
        nullable=True, comment="Whether a follow-up appointment is needed"
    )

    follow_up_instructions: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Instructions for follow-up care"
    )

    next_appointment_recommended_days: Mapped[Optional[int]] = mapped_column(
        nullable=True, comment="Recommended days until next appointment"
    )
    ...
    __table_args__ = (
        ...
        Index(
            ...
            postgresql_where=(
                priority.in_(
                    [AppointmentPriority.URGENT, AppointmentPriority.EMERGENCY]
                )
            ),
        ),
        ...
        Index(
            "idx_appointments_service_other_description",
    ...
    def is_emergency(self) -> bool:
        ...
        return (
            self.priority == AppointmentPriority.EMERGENCY
            or self.service_type == ServiceType.EMERGENCY
        )
    ...
    def mark_no_show(self) -> None:
        ...
        if self.status not in {
            ...
            raise ValueError(
                f"Cannot mark no-show for appointment with status {self.status.value}"
            )
    ...
    def get_service_display(self) -> str:
        ...
        if (
            ...
            and self.service_type_other_description
        ):
    ...
    def set_other_service_type(self, description: str) -> None:
        ...
        if not description or not description.strip():
            raise ValueError(
                "Description is required when setting service type to 'other'"
            )
        self.service_type = ServiceType.OTHER
    ...
    def validate_service_type_description(self) -> bool:
        ...
        if self.service_type == ServiceType.OTHER:
            return bool(
                self.service_type_other_description
                and self.service_type_other_description.strip()
            )
        return True
