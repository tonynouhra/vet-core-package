...
from sqlalchemy import (
    Boolean, CheckConstraint, Date, Enum, ForeignKey, Index, Numeric, String, Text
)
...
class Pet(BaseModel):
    ...
    birth_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="Pet's birth date"
    )
    
    approximate_age_years: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Approximate age in years if birth date unknown"
    )
    
    approximate_age_months: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Approximate age in months if birth date unknown"
    )
    ...
    registration_number: Mapped[Optional[str]] = mapped_column(
        String(100),
        nullable=True,
        comment="Breed registration number"
    )
    ...
    spay_neuter_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="Date when pet was spayed or neutered"
    )
    ...
    microchip_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="Date when microchip was implanted"
    )
    ...
    temperament: Mapped[Optional[str]] = mapped_column(
        String(200),
        nullable=True,
        comment="Pet's temperament description"
    )
    
    special_needs: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Special care needs or requirements"
    )
    
    behavioral_notes: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Behavioral notes and observations"
    )
    ...
    profile_photo_url: Mapped[Optional[str]] = mapped_column(
        String(500),
        nullable=True,
        comment="URL to pet's profile photo"
    )
    
    additional_photos: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Array of additional photo URLs"
    )
    ...
    medical_history: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Comprehensive medical history stored as JSON"
    )
    ...
    emergency_contact: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Emergency contact information stored as JSON"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_pets_microchip_active', 'microchip_id', postgresql_where=(status == PetStatus.ACTIVE)),
        ...
        Index('idx_pets_medical_history_gin', 'medical_history', postgresql_using='gin'),
        Index('idx_pets_vaccination_records_gin', 'vaccination_records', postgresql_using='gin'),
        Index('idx_pets_medication_history_gin', 'medication_history', postgresql_using='gin'),
        Index('idx_pets_allergy_information_gin', 'allergy_information', postgresql_using='gin'),
    ...
    def is_due_for_vaccination(self, vaccine_type: str) -> bool:
        ...
        for record in self.vaccination_records:
            if record.get('vaccine_type') == vaccine_type:
                ...
                if vaccination_date:
                    if latest_vaccination is None or vaccination_date > latest_vaccination:
    ...
    def add_vaccination_record(self, vaccine_type: str, date_administered: date, 
                             veterinarian: str, batch_number: Optional[str] = None,
                             next_due_date: Optional[date] = None, notes: Optional[str] = None) -> None:
        """
    ...
    def add_medical_record(self, record_type: str, description: str, 
                          date_recorded: date, veterinarian: Optional[str] = None,
                          diagnosis: Optional[str] = None, treatment: Optional[str] = None,
                          follow_up_needed: bool = False) -> None:
        """
    ...
    def add_allergy(self, allergen: str, reaction: str, severity: str = "unknown",
                   date_discovered: Optional[date] = None) -> None:
        """
    ...
    def update_weight(self, weight_kg: Decimal, recorded_by: Optional[str] = None) -> None:
        """
    ...
    def update_weight_from_lbs(self, weight_lbs: Decimal, recorded_by: Optional[str] = None) -> None:
        """
    ...
    def mark_deceased(self, date_of_death: Optional[date] = None, cause: Optional[str] = None) -> None:
        """
        ...
        self.medical_history['deceased'] = {
            'date_of_death': date_of_death.isoformat() if date_of_death else date.today().isoformat(),
            'cause': cause,
            'recorded_at': datetime.utcnow().isoformat()
        }
    
    def transfer_ownership(self, new_owner_id: uuid.UUID, transfer_reason: Optional[str] = None) -> None:
        """
    ...
    def get_medical_records_by_type(self, record_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            record for record in self.medical_history['records']
