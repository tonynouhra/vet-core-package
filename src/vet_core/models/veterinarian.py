...
from sqlalchemy import (
    Boolean, CheckConstraint, Date, Enum, ForeignKey, Index, Numeric, String, Text, UniqueConstraint
)
...
class Veterinarian(BaseModel):
    ...
    license_issued_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="Date when license was first issued"
    )
    ...
    license_expiry_date: Mapped[Optional[date]] = mapped_column(
        ...
        nullable=True,
        index=True,
        comment="Date when license expires"
    )
    ...
    veterinary_school: Mapped[Optional[str]] = mapped_column(
        String(200),
        nullable=True,
        comment="Name of veterinary school attended"
    )
    
    graduation_year: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Year of graduation from veterinary school"
    )
    
    degree_type: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="Type of veterinary degree (DVM, VMD, etc.)"
    )
    ...
    years_of_experience: Mapped[int] = mapped_column(
        nullable=False,
        default=0,
        comment="Years of veterinary experience"
    )
    
    hire_date: Mapped[Optional[date]] = mapped_column(
        Date,
        nullable=True,
        comment="Date hired at current clinic"
    )
    ...
    services_provided: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="List of services this veterinarian provides"
    )
    ...
    availability: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Weekly availability schedule stored as JSON"
    )
    ...
    max_daily_appointments: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Maximum number of appointments per day"
    )
    ...
    bio: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Professional biography and background"
    )
    
    professional_interests: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Professional interests and research areas"
    )
    
    languages_spoken: Mapped[Optional[List[str]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Languages spoken by the veterinarian"
    )
    ...
    professional_memberships: Mapped[Optional[List[Dict[str, Any]]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Professional organizations and memberships"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name='ck_veterinarians_experience_non_negative'
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            ...
            name='ck_veterinarians_rating_valid'
        ),
        CheckConstraint(
            'total_reviews >= 0',
            name='ck_veterinarians_reviews_non_negative'
        ),
        CheckConstraint(
        ...
        Index('idx_veterinarians_status_accepting', 'status', 'is_accepting_new_patients'),
        ...
        Index('idx_veterinarians_license_expiry', 'license_expiry_date', 'license_status'),
        ...
        Index('idx_veterinarians_specializations_gin', 'specializations', postgresql_using='gin'),
        Index('idx_veterinarians_services_gin', 'services_provided', postgresql_using='gin'),
        Index('idx_veterinarians_species_gin', 'species_expertise', postgresql_using='gin'),
        Index('idx_veterinarians_availability_gin', 'availability', postgresql_using='gin'),
        Index('idx_veterinarians_certifications_gin', 'additional_certifications', postgresql_using='gin'),
        Index('idx_veterinarians_memberships_gin', 'professional_memberships', postgresql_using='gin'),
        Index('idx_veterinarians_languages_gin', 'languages_spoken', postgresql_using='gin'),
        
        # Full-text search indexes
        Index('idx_veterinarians_bio_search', 'bio', postgresql_using='gin', postgresql_ops={'bio': 'gin_trgm_ops'}),
        Index('idx_veterinarians_school_search', 'veterinary_school', postgresql_using='gin', postgresql_ops={'veterinary_school': 'gin_trgm_ops'}),
    )
    ...
    def is_active(self) -> bool:
        ...
        return (
            self.status == VeterinarianStatus.ACTIVE and
    ...
    def is_available_for_appointments(self) -> bool:
        ...
        return (
            self.is_active and
            self.is_accepting_new_patients
        )
    ...
    def has_specialization(self, specialization: str) -> bool:
        ...
        """
        return self.specializations is not None and specialization in self.specializations
    ...
    def set_availability(self, day_of_week: str, start_time: str, end_time: str,
                        is_available: bool = True, break_times: Optional[List[Dict[str, str]]] = None) -> None:
        """
    ...
    def add_certification(self, certification_name: str, issuing_organization: str,
                         date_obtained: date, expiry_date: Optional[date] = None,
                         certification_number: Optional[str] = None) -> None:
        """
    ...
    def update_license_status(self, new_status: LicenseStatus, reason: Optional[str] = None) -> None:
        """
        ...
        if new_status in {LicenseStatus.EXPIRED, LicenseStatus.SUSPENDED, LicenseStatus.REVOKED}:
    ...
    def get_license_status_display(self) -> str:
        ...
        return status_display.get(self.license_status, self.license_status.value.title())
    ...
    def get_employment_type_display(self) -> str:
        ...
        return type_display.get(self.employment_type, self.employment_type.value.title())
    
    def calculate_capacity_utilization(self, current_appointments: int) -> Optional[float]:
        """
