...
def validate_email(email: str) -> ValidationResult[str]:
    ...
    if not EMAIL_PATTERN.match(sanitized_email):
        result.add_error(ValidationError("Invalid email format", "email", "invalid_format"))
        return result
...
def validate_phone(phone: str, country: str = 'us') -> ValidationResult[str]:
    ...
    if not phone:
        result.add_error(ValidationError("Phone number is required", "phone", "required"))
        return result
    ...
    if not pattern.match(phone):
        result.add_error(ValidationError("Invalid phone number format", "phone", "invalid_format"))
        return result
    ...
    if country == 'us' and len(digits_only) >= 10:
        ...
        if len(digits_only) == 10:
            ...
            result.add_error(ValidationError("Invalid US phone number", "phone", "invalid_format"))
    else:
        result.value = phone
...
def validate_weight(weight: Union[str, float, Decimal], unit: str = 'lbs') -> ValidationResult[Decimal]:
    """
    ...
    try:
        ...
        result.add_error(ValidationError("Weight must be a valid number", "weight", "invalid_number"))
        return result
    ...
    if unit == 'lbs':
        ...
        result.add_error(ValidationError("Invalid weight unit", "weight", "invalid_unit"))
        return result
    
    if weight_decimal < min_weight:
        result.add_error(ValidationError(f"Weight must be at least {min_weight} {unit}", "weight", "too_low"))
        return result
    
    if weight_decimal > max_weight:
        result.add_error(ValidationError(f"Weight cannot exceed {max_weight} {unit}", "weight", "too_high"))
        return result
...
def validate_license_number(license_number: str, pattern_type: str = 'us_general') -> ValidationResult[str]:
    """
    ...
    if not license_number:
        result.add_error(ValidationError("License number is required", "license_number", "required"))
        return result
    
    # Sanitize and uppercase
    sanitized = sanitize_string(license_number).upper().replace(' ', '').replace('-', '')
    
    # Validate pattern
    pattern = LICENSE_PATTERNS.get(pattern_type, LICENSE_PATTERNS['us_general'])
    
    if not pattern.match(sanitized):
        result.add_error(ValidationError("Invalid license number format", "license_number", "invalid_format"))
        return result
...
def validate_age_range(birth_date: date, min_age_days: int = 0, max_age_years: int = 30) -> ValidationResult[date]:
    """
    ...
    if not birth_date:
        result.add_error(ValidationError("Birth date is required", "birth_date", "required"))
        return result
    ...
    if birth_date > today:
        result.add_error(ValidationError("Birth date cannot be in the future", "birth_date", "future_date"))
        return result
    
    # Check minimum age
    min_date = today - timedelta(days=min_age_days)
    if birth_date > min_date:
        result.add_error(ValidationError(f"Pet must be at least {min_age_days} days old", "birth_date", "too_young"))
        return result
    
    # Check maximum age
    max_date = today.replace(year=today.year - max_age_years)
    if birth_date < max_date:
        result.add_error(ValidationError(f"Pet age cannot exceed {max_age_years} years", "birth_date", "too_old"))
        return result
...
def validate_species_breed(species: str, breed: str) -> ValidationResult[Dict[str, str]]:
    """
    ...
    valid_species = ['Dog', 'Cat', 'Bird', 'Rabbit', 'Hamster', 'Guinea Pig', 'Ferret', 'Reptile', 'Fish', 'Other']
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(value: str, *args, **kwargs):
            if len(value) < min_length:
                raise ValidationError(f"Value must be at least {min_length} characters", code="too_short")
            if max_length and len(value) > max_length:
                raise ValidationError(f"Value cannot exceed {max_length} characters", code="too_long")
            return func(value, *args, **kwargs)
        return wrapper
    return decorator


def validate_range(min_value: Optional[Union[int, float]] = None, max_value: Optional[Union[int, float]] = None):
    """Decorator to validate numeric ranges."""
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(value: Union[int, float], *args, **kwargs):
            if min_value is not None and value < min_value:
                raise ValidationError(f"Value must be at least {min_value}", code="too_low")
            if max_value is not None and value > max_value:
                raise ValidationError(f"Value cannot exceed {max_value}", code="too_high")
            return func(value, *args, **kwargs)
        return wrapper
    return decorator
...
def batch_validate(validators: Dict[str, Callable], data: Dict[str, Any]) -> ValidationResult[Dict[str, Any]]:
    """
    ...
    for field_name, validator in validators.items():
        if field_name in data:
            try:
                ...
                result.add_error(ValidationError(str(e), field_name, "validation_error"))
