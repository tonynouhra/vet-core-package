...
class BusinessHours:
    ...
    def __init__(self, day: DayOfWeek, open_time: time, close_time: time, is_closed: bool = False):
        self.day = day
...
def is_business_hours(
    ...
    local_dt = from_utc(check_datetime, timezone) if check_datetime.tzinfo else check_datetime
...
def get_available_appointment_slots(
    ...
    start_datetime = datetime.combine(date_to_check, hours.open_time, ZoneInfo(timezone))
...
def calculate_pet_age(birth_date: date, reference_date: Optional[date] = None) -> Dict[str, int]:
    """
    ...
    if days < 0:
        ...
        if reference_date.month == 1:
            ...
            prev_month_last_day = calendar.monthrange(reference_date.year, reference_date.month - 1)[1]
...
def get_pet_age_category(birth_date: date, reference_date: Optional[date] = None) -> str:
    """
...
def is_appointment_time_valid(
    ...
    if appointment_utc < min_time:
        return False, f"Appointment must be scheduled at least {min_advance_hours} hours in advance"
    
    # Check maximum advance time
    max_time = now + timedelta(days=max_advance_days)
    if appointment_utc > max_time:
        return False, f"Appointment cannot be scheduled more than {max_advance_days} days in advance"
...
def round_to_nearest_slot(
    dt: datetime,
    slot_duration: timedelta = timedelta(minutes=15)
) -> datetime:
    """
