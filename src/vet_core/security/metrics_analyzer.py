...
from .status_tracker import VulnerabilityStatusTracker, VulnerabilityStatus, ProgressStage
...
class SecurityMetricsAnalyzer:
    ...
    def __init__(
        ...
        self.logger.info(f"Initialized SecurityMetricsAnalyzer with database: {self.metrics_db_path}")
    ...
    def _init_metrics_database(self) -> None:
        ...
        try:
            ...
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                # Create indexes
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_date ON metrics_snapshots(snapshot_date)")
    ...
    def calculate_current_metrics(
        ...
        self.logger.info(f"Calculating security metrics for period: {start_date} to {end_date}")
        
        # Get audit events for the period
        events = self.audit_trail.get_audit_events(start_date=start_date, end_date=end_date)
        
        # Get current tracking records
        tracking_records = self.status_tracker.get_all_tracking_records()
        
        # Calculate vulnerability metrics
        vulnerability_metrics = self._calculate_vulnerability_metrics(events, tracking_records)
        
        # Calculate time-based metrics
        time_metrics = self._calculate_time_based_metrics(events, tracking_records)
        
        # Calculate performance metrics
        performance_metrics = self._calculate_performance_metrics(events, tracking_records, period_days)
    ...
    def _calculate_vulnerability_metrics(
        ...
        for record in tracking_records:
            ...
            if record.current_status in [VulnerabilityStatus.RESOLVED, VulnerabilityStatus.CLOSED]:
    ...
    def _calculate_time_based_metrics(
        ...
        for vuln_id, vuln_events in vulnerability_timelines.items():
            ...
            if detected_time and assessed_time:
                assessment_times.append((assessed_time - detected_time).total_seconds() / 3600)
            
            if detected_time and resolved_time:
                resolution_times.append((resolved_time - detected_time).total_seconds() / 3600)
        
        # Calculate means
        return {
            "detection": None,  # Would need external data source
            "assessment": statistics.mean(assessment_times) if assessment_times else None,
            "resolution": statistics.mean(resolution_times) if resolution_times else None,
            "verification": statistics.mean(verification_times) if verification_times else None,
        }
    ...
    def _calculate_performance_metrics(
        ...
        """Calculate performance metrics."""
        # Count scans
        scan_events = [e for e in events if e.event_type == AuditEventType.SCAN_COMPLETED]
        scan_frequency = len(scan_events) / period_days if period_days > 0 else 0.0
        
        # Calculate detection rate
        vulnerability_events = [e for e in events if e.event_type == AuditEventType.VULNERABILITY_DETECTED]
        detection_rate = len(vulnerability_events) / len(scan_events) if scan_events else 0.0
        
        # Calculate resolution rate
        resolved_count = sum(
            1 for record in tracking_records 
            if record.current_status in [VulnerabilityStatus.RESOLVED, VulnerabilityStatus.CLOSED]
        )
        resolution_rate = (resolved_count / len(tracking_records) * 100) if tracking_records else 0.0
    ...
    def _calculate_compliance_metrics(self, tracking_records: List) -> Dict[str, Any]:
        ...
        sla_compliance_rate = ((len(tracking_records) - overdue_count) / len(tracking_records) * 100)
    ...
    def _calculate_trend_indicators(self, period_days: int) -> Dict[str, str]:
        ...
        historical_metrics = self._get_historical_metrics(period_days * 2)  # Double period for comparison
        ...
        # Vulnerability trend
        recent_vuln_avg = statistics.mean([m.total_vulnerabilities for m in recent_metrics])
        older_vuln_avg = statistics.mean([m.total_vulnerabilities for m in older_metrics])
    ...
    def analyze_trends(
        ...
        for metric_name in metric_names:
            ...
            growth_rate = ((values[-1] - values[0]) / values[0] * 100) if values[0] != 0 else 0.0
    ...
    def generate_metrics_report(
        ...
        if include_historical:
            historical_metrics = self._get_historical_metrics(period_days * 3)  # 3x period for context
            report["historical_data"] = [metrics.to_dict() for metrics in historical_metrics[-30:]]  # Last 30 snapshots
        
        # Add insights and recommendations
        report["insights"] = self._generate_insights(current_metrics, report.get("trend_analysis", []))
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    metrics = SecurityMetrics(
                        total_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("total_vulnerabilities", 0),
                        critical_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("critical_vulnerabilities", 0),
                        high_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("high_vulnerabilities", 0),
                        medium_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("medium_vulnerabilities", 0),
                        low_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("low_vulnerabilities", 0),
                        resolved_vulnerabilities=metrics_data.get("vulnerability_metrics", {}).get("resolved_vulnerabilities", 0),
                        scan_frequency=metrics_data.get("performance_metrics", {}).get("scan_frequency", 0.0),
                        resolution_rate=metrics_data.get("performance_metrics", {}).get("resolution_rate", 0.0),
                        sla_compliance_rate=metrics_data.get("compliance_metrics", {}).get("sla_compliance_rate", 0.0),
                        calculated_at=datetime.fromisoformat(metrics_data.get("metadata", {}).get("calculated_at", datetime.now().isoformat())),
                        calculation_period=metrics_data.get("metadata", {}).get("calculation_period", "30_days"),
                    )
    ...
    def _generate_insights(
        ...
        for trend in trend_analysis:
            if trend["trend_direction"] == "increasing" and trend["metric_name"] == "total_vulnerabilities":
                insights.append("📈 Vulnerability count is trending upward - consider increasing scan frequency")
            elif trend["trend_direction"] == "decreasing" and trend["metric_name"] == "resolution_rate":
                insights.append("📉 Resolution rate is declining - review remediation processes")
    ...
    def _generate_recommendations(self, current_metrics: SecurityMetrics) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("Continue current security practices and monitor for changes")
