...
class ProgressMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_in_current_status_hours": self.time_in_current_status.total_seconds() / 3600,
            "total_time_since_detection_hours": self.total_time_since_detection.total_seconds() / 3600,
...
class VulnerabilityStatusTracker:
    ...
    STATUS_PROGRESSION = {
        ...
        VulnerabilityStatus.DETECTED: [VulnerabilityStatus.ASSESSED, VulnerabilityStatus.IGNORED, VulnerabilityStatus.ASSIGNED],
        VulnerabilityStatus.ASSESSED: [VulnerabilityStatus.ASSIGNED, VulnerabilityStatus.DEFERRED, VulnerabilityStatus.IN_PROGRESS],
        VulnerabilityStatus.ASSIGNED: [VulnerabilityStatus.IN_PROGRESS, VulnerabilityStatus.ASSESSED],
        VulnerabilityStatus.IN_PROGRESS: [VulnerabilityStatus.TESTING, VulnerabilityStatus.RESOLVED, VulnerabilityStatus.ASSIGNED],
        VulnerabilityStatus.TESTING: [VulnerabilityStatus.RESOLVED, VulnerabilityStatus.IN_PROGRESS],
        VulnerabilityStatus.RESOLVED: [VulnerabilityStatus.VERIFIED, VulnerabilityStatus.IN_PROGRESS, VulnerabilityStatus.CLOSED],
    ...
    def __init__(
        ...
        self.logger.info(f"Initialized VulnerabilityStatusTracker with database: {self.tracking_db_path}")
    ...
    def _init_tracking_database(self) -> None:
        ...
        try:
            ...
            with sqlite3.connect(self.tracking_db_path) as conn:
                ...
                # Create indexes
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_tracking_status ON vulnerability_tracking(current_status)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_tracking_severity ON vulnerability_tracking(severity)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_tracking_updated ON vulnerability_tracking(updated_at)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_status_changes_vuln ON status_changes(vulnerability_id)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_status_changes_time ON status_changes(changed_at)")
    ...
    def update_status(
        ...
        if not record:
            self.logger.error(f"Vulnerability {vulnerability_id} not found for status update")
            return False
    ...
    def get_tracking_record(self, vulnerability_id: str) -> Optional[VulnerabilityTrackingRecord]:
        """
    ...
    def get_progress_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress_metrics": {
                ...
                "completion_rate": (completed_count / len(all_records) * 100) if all_records else 0.0,
            },
    ...
    def get_overdue_vulnerabilities(self) -> List[VulnerabilityTrackingRecord]:
        ...
        overdue_records.sort(
            key=lambda r: r.progress_metrics.sla_deadline if r.progress_metrics.sla_deadline else datetime.now(),
            reverse=False
        )
    ...
    def _calculate_progress_metrics(self, record: VulnerabilityTrackingRecord) -> ProgressMetrics:
        ...
        if record.current_status not in [VulnerabilityStatus.CLOSED, VulnerabilityStatus.IGNORED]:
            remaining_progress = 100.0 - progress_percentage
            if remaining_progress > 0:
                # Estimate based on average time per progress unit
                avg_time_per_percent = total_time_since_detection.total_seconds() / max(progress_percentage, 1.0)
        ...
        elif record.current_status in [VulnerabilityStatus.RESOLVED, VulnerabilityStatus.VERIFIED]:
    ...
    def _save_tracking_record(self, record: VulnerabilityTrackingRecord) -> None:
        ...
        try:
                ...
                if record.progress_metrics:
                    ...
                    """, (
                        ...
                        (record.progress_metrics.estimated_completion_time.isoformat() 
                         if record.progress_metrics.estimated_completion_time else None),
                        (record.progress_metrics.sla_deadline.isoformat() 
                         if record.progress_metrics.sla_deadline else None),
            ...
            self.logger.error(f"Failed to save tracking record {record.vulnerability_id}: {e}")
            raise
    ...
    def _load_tracking_record(self, vulnerability_id: str) -> Optional[VulnerabilityTrackingRecord]:
            ...
            with sqlite3.connect(self.tracking_db_path) as conn:
                ...
                for change_row in cursor.fetchall():
                    change = StatusChange(
                        ...
                        vulnerability_id=vulnerability_id,
                        old_status=VulnerabilityStatus(change_row[1]) if change_row[1] else None,
