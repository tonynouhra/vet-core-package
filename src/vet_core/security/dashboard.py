...
class VulnerabilityDashboard:
    ...
    def __init__(
        ...
        self.metrics_analyzer = SecurityMetricsAnalyzer(self.audit_trail, self.status_tracker)
    ...
    def scan_vulnerabilities(
        ...
        try:
            ...
            print(f"✅ Scan completed: {report.vulnerability_count} vulnerabilities found")
    ...
    def _show_single_vulnerability_status(self, vulnerability_id: str) -> None:
        ...
        print(f"Fix Versions: {', '.join(vulnerability.fix_versions) if vulnerability.fix_versions else 'None'}")
        print(f"Description: {vulnerability.description[:100]}..." if len(vulnerability.description) > 100 else vulnerability.description)
        ...
        if tracking_record:
            ...
            if tracking_record.progress_metrics:
                ...
                print(f"SLA Status: {'⚠️ OVERDUE' if metrics.is_overdue else '✅ On Track'}")
    ...
    def _show_all_vulnerabilities_status(self) -> None:
        ...
        print(f"Scan Date: {self.current_report.scan_date.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def assess_risks(self, show_details: bool = False) -> None:
        ...
        prioritized = self.risk_assessor.get_prioritized_vulnerabilities(self.current_report)
    ...
    def _generate_summary_report(self, output_file: Optional[Path], format_type: str) -> None:
        ...
        if format_type == "json":
            self._save_json_report(report_data, output_file or Path("vulnerability_summary.json"))
        else:
            self._print_text_report(report_data, "Summary Report")

    def _generate_detailed_report(self, output_file: Optional[Path], format_type: str) -> None:
        ...
        report_data = {
            ...
            "vulnerability_details": [vuln.to_dict() for vuln in self.current_report.vulnerabilities]
        }
        
        if format_type == "json":
            self._save_json_report(report_data, output_file or Path("vulnerability_detailed.json"))
        else:
            self._print_detailed_text_report(report_data)

    def _generate_compliance_report(self, output_file: Optional[Path], format_type: str) -> None:
        """Generate compliance report."""
        violations, metrics = self.compliance_manager.check_compliance(self.current_report)
        ...
        if format_type == "json":
            self._save_json_report(compliance_report, output_file or Path("compliance_report.json"))
        else:
            self._print_compliance_text_report(compliance_report, violations, metrics)

    def _generate_trends_report(self, output_file: Optional[Path], format_type: str) -> None:
    ...
    def _analyze_security_trends(self, events: List) -> Dict[str, Any]:
        ...
        total_detected = sum(day["vulnerabilities_detected"] for day in daily_stats.values())
        total_resolved = sum(day["vulnerabilities_resolved"] for day in daily_stats.values())
        ...
        return {
            ...
            "summary_metrics": {
                ...
                "resolution_rate": (total_resolved / total_detected * 100) if total_detected > 0 else 0,
    ...
    def _update_vulnerability_status(self, report: SecurityReport) -> None:
        ...
        for vuln in report.vulnerabilities:
            ...
            if not existing_record:
                ...
                self.vulnerability_status[vuln.id] = existing_record.current_status.value
    ...
    def _print_compliance_text_report(self, report: Dict[str, Any], violations: List, metrics) -> None:
        ...
        print(f"Overall Compliance Score: {compliance_overview.get('overall_score', 0):.1f}%")
        print(f"Policy Violations: {compliance_overview.get('policy_violations', 0)}")
        print(f"Active Policy Rules: {compliance_overview.get('active_policy_rules', 0)}")
    ...
    def _print_metrics_text_report(self, report: Dict[str, Any]) -> None:
        ...
        print(f"Analysis Period: {report['report_metadata']['analysis_period_days']} days")
    ...
    def _print_trends_text_report(self, data: Dict[str, Any]) -> None:
        ...
        print(f"Analysis Period: {data['analysis_period']['start_date']} to {data['analysis_period']['end_date']}")
        ...
        print(f"Mean Resolution Time: {summary['mean_resolution_time_hours']:.1f} hours")
    ...
    def update_vulnerability_status(
        ...
        try:
            ...
            print("Valid statuses: new, detected, assessed, assigned, in_progress, testing, resolved, verified, closed, ignored, deferred")
    ...
    def show_overdue_vulnerabilities(self) -> None:
        ...
        for record in overdue[:10]:  # Show first 10
            ...
            days_overdue = (datetime.now() - metrics.sla_deadline).days if metrics.sla_deadline else 0
...
def create_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--audit-db",
        type=Path,
        help="Path to audit database file"
    )
    ...
    scan_parser.add_argument("--no-description", action="store_true", help="Skip vulnerability descriptions")
    
    # Status command
    status_parser = subparsers.add_parser("status", help="Show vulnerability status")
    status_parser.add_argument("vulnerability_id", nargs="?", help="Specific vulnerability ID")
    
    # Assess command
    assess_parser = subparsers.add_parser("assess", help="Perform risk assessment")
    assess_parser.add_argument("--details", action="store_true", help="Show detailed assessment")
...
def main() -> int:
    """Main entry point for the vulnerability dashboard CLI."""
    ...
    while True:
        try:
            ...
            if cmd == "exit":
                ...
                if len(parts) < 2:
                    print("❌ Report type required. Use: report <summary|detailed|compliance|trends>")
                    continue
                report_type = parts[1]
