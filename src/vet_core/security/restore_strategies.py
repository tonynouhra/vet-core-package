...
class ForceReinstallStrategy(RestoreStrategy):
    ...
    def restore(self, backup: EnvironmentBackup) -> RestoreResult:
        ...
        try:
            ...
            pre_check_result = secure_subprocess_run(
                pre_check_cmd, capture_output=True, text=True
            )
            ...
            post_check_result = secure_subprocess_run(
                post_check_cmd, capture_output=True, text=True
            )
    ...
    def restore(self, backup: EnvironmentBackup) -> RestoreResult:
        ...
        try:
            ...
            if not packages:
                ...
                if backup.is_empty_environment:
                    warnings.append(
                        "Environment restored to empty state - no packages were installed"
                    )
                return RestoreResult.success_result(
...
class FallbackStrategy(RestoreStrategy):
    ...
    def restore(self, backup: EnvironmentBackup) -> RestoreResult:
        ...
        try:
            ...
            successful_packages, failed_packages = self._try_individual_packages(
                packages
            )
