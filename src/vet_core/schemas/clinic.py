...
class OperatingHoursSchema(BaseModel):
    ...
    lunch_break: Optional[Dict[str, str]] = Field(None, description="Lunch break with 'start' and 'end' times")
    ...
    @classmethod
    def validate_lunch_break(cls, v: Optional[Dict[str, str]]) -> Optional[Dict[str, str]]:
    ...
    def validate_operating_hours_consistency(self):
        ...
        if self.is_open:
            if not self.open_time or not self.close_time:
                raise ValueError("Open and close times are required when clinic is open")
...
class ClinicBase(BaseModel):
    ...
    type: ClinicType = Field(
        ...
        description="Type of veterinary clinic"
    )
    ...
    specialties: Optional[List[str]] = Field(
        None,
        description="List of medical specialties available"
    )
    accepts_new_patients: bool = Field(
        True,
        description="Whether the clinic is accepting new patients"
    )
    accepts_emergencies: bool = Field(
        False,
        description="Whether the clinic handles emergency cases"
    )
    accepts_walk_ins: bool = Field(
        False,
        description="Whether the clinic accepts walk-in appointments"
    )
    description: Optional[str] = Field(
        None,
        description="Description of the clinic and its services"
    )
    ...
    equipment_available: Optional[List[str]] = Field(
        None,
        description="List of medical equipment available"
    )
    ...
    number_of_exam_rooms: Optional[int] = Field(
        None,
        description="Number of examination rooms",
        gt=0
    )
    number_of_surgery_rooms: Optional[int] = Field(
        None,
        description="Number of surgery rooms",
        ge=0
    )
    ...
    after_hours_instructions: Optional[str] = Field(
        None,
        description="Instructions for after-hours emergencies"
    )
    logo_url: Optional[str] = Field(
        None,
        description="URL to clinic logo image",
        max_length=500
    )
    photos: Optional[List[str]] = Field(
        None,
        description="Array of photo URLs for the clinic"
    )
    ...
    def validate_timezone(cls, v: Optional[str]) -> Optional[str]:
        ...
        if not re.match(r'^[A-Za-z_]+/[A-Za-z_]+$', v):
            raise ValueError("Timezone must be in format 'Region/City' (e.g., 'America/New_York')")
        
        return v
    
    @field_validator('services_offered', 'specialties', 'facility_features', 'equipment_available', 'insurance_accepted', 'payment_methods')
    @classmethod
...
class ClinicCreate(ClinicBase):
    ...
    license_number: Optional[str] = Field(
        None,
        description="Veterinary clinic license number",
        max_length=100
    )
    status: ClinicStatus = Field(
        ClinicStatus.ACTIVE,
        description="Initial clinic status"
    )
    ...
    @classmethod
    def validate_operating_hours(cls, v: Optional[Dict[str, OperatingHoursSchema]]) -> Optional[Dict[str, OperatingHoursSchema]]:
        ...
        for day, hours in v.items():
            ...
            if not isinstance(hours, OperatingHoursSchema):
                raise ValueError(f"Operating hours for {day} must be an OperatingHoursSchema")
...
class ClinicUpdate(BaseModel):
    ...
    type: Optional[ClinicType] = Field(
        None,
        description="Type of veterinary clinic"
    )
    status: Optional[ClinicStatus] = Field(
        None,
    ...
    _validate_phone_number = field_validator('phone_number', 'emergency_contact_number')(ClinicBase.validate_phone_number)
    _validate_url_format = field_validator('website_url', 'logo_url')(ClinicBase.validate_url_format)
    _validate_postal_code = field_validator('postal_code')(ClinicBase.validate_postal_code)
    ...
    _validate_string_lists = field_validator('services_offered', 'specialties', 'facility_features', 'equipment_available', 'insurance_accepted', 'payment_methods')(ClinicBase.validate_string_lists)
    _validate_photos = field_validator('photos')(ClinicBase.validate_photos)
    _validate_text_fields = field_validator('description', 'after_hours_instructions')(ClinicBase.validate_text_fields)
    _validate_license_number = field_validator('license_number')(ClinicCreate.validate_license_number)
    _validate_operating_hours = field_validator('operating_hours')(ClinicCreate.validate_operating_hours)
...
class ClinicResponse(BaseModel):
    ...
    license_number: Optional[str] = Field(None, description="Veterinary clinic license number")
    ...
    specialties: Optional[List[str]] = Field(None, description="List of medical specialties")
    accepts_new_patients: bool = Field(..., description="Whether accepting new patients")
    accepts_emergencies: bool = Field(..., description="Whether handling emergency cases")
    accepts_walk_ins: bool = Field(..., description="Whether accepting walk-in appointments")
    description: Optional[str] = Field(None, description="Clinic description")
    facility_features: Optional[List[str]] = Field(None, description="List of facility features")
    equipment_available: Optional[List[str]] = Field(None, description="List of medical equipment")
    max_daily_appointments: Optional[int] = Field(None, description="Maximum daily appointments")
    number_of_exam_rooms: Optional[int] = Field(None, description="Number of examination rooms")
    number_of_surgery_rooms: Optional[int] = Field(None, description="Number of surgery rooms")
    insurance_accepted: Optional[List[str]] = Field(None, description="Accepted insurance providers")
    ...
    after_hours_instructions: Optional[str] = Field(None, description="After-hours instructions")
...
class ClinicStatusUpdate(BaseModel):
    ...
    def validate_status(cls, v: ClinicStatus) -> ClinicStatus:
        ...
        if v not in ClinicStatus:
            raise ValueError(f"Invalid status. Must be one of: {[status.value for status in ClinicStatus]}")
        return v
...
class ClinicSearchFilters(BaseModel):
    ...
    accepts_new_patients: Optional[bool] = Field(None, description="Filter by new patient acceptance")
    accepts_emergencies: Optional[bool] = Field(None, description="Filter by emergency services")
    accepts_walk_ins: Optional[bool] = Field(None, description="Filter by walk-in acceptance")
    services_offered: Optional[List[str]] = Field(None, description="Filter by services offered")
    specialties: Optional[List[str]] = Field(None, description="Filter by specialties")
    latitude: Optional[float] = Field(None, description="Latitude for distance search", ge=-90, le=90)
    longitude: Optional[float] = Field(None, description="Longitude for distance search", ge=-180, le=180)
    radius_km: Optional[float] = Field(None, description="Search radius in kilometers", gt=0, le=100)
    ...
    def validate_location_search(self):
        ...
        if provided_count > 0 and provided_count != 3:
            raise ValueError("For location search, latitude, longitude, and radius_km must all be provided")
...
class ClinicOperatingHoursUpdate(BaseModel):
    ...
    @field_validator('operating_hours')
    @classmethod
    def validate_operating_hours(cls, v: Dict[str, OperatingHoursSchema]) -> Dict[str, OperatingHoursSchema]:
        """Validate operating hours."""
        valid_days = {'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'}
...
class ClinicLocationUpdate(BaseModel):
    ...
    address_line2: Optional[str] = Field(None, description="Secondary address line", max_length=255)
    ...
    timezone: Optional[str] = Field(None, description="Timezone identifier", max_length=50)
