...
from ..models.appointment import AppointmentStatus, ServiceType, AppointmentPriority
...
class AppointmentBase(BaseModel):
    ...
    service_type_other_description: Optional[str] = Field(
        None,
        description="Description when service_type is 'other'",
        max_length=200
    )
    ...
    def validate_scheduled_at(cls, v: datetime) -> datetime:
        ...
        if v > max_future:
            raise ValueError("Appointment cannot be scheduled more than 2 years in advance")
    ...
    def validate_service_type_consistency(self):
        """Validate that service_type 'other' has description."""
        if self.service_type == ServiceType.OTHER and not self.service_type_other_description:
            raise ValueError("Service type other description is required when service type is 'other'")
        return self
    ...
    def validate_business_hours(self):
        ...
        if day_of_week == 6:  # Sunday
            if not 9 <= hour <= 17:  # 9 AM to 5 PM on Sunday
                raise ValueError("Regular appointments on Sunday must be between 9 AM and 5 PM")
...
class AppointmentCreate(AppointmentBase):
    ...
    pet_id: UUID = Field(
        ...,
        description="UUID of the pet for this appointment"
    )
    veterinarian_id: UUID = Field(
        ...,
        description="UUID of the assigned veterinarian"
    )
    ...
    def validate_initial_status(cls, v: AppointmentStatus) -> AppointmentStatus:
        ...
        if v not in allowed_initial_statuses:
            raise ValueError(f"Initial status must be one of: {[s.value for s in allowed_initial_statuses]}")
...
class AppointmentUpdate(BaseModel):
    ...
    internal_notes: Optional[str] = Field(
        None,
        description="Internal staff notes not visible to pet owner"
    )
    estimated_cost: Optional[Decimal] = Field(
        None,
        description="Estimated cost of the appointment",
    ...
    actual_cost: Optional[Decimal] = Field(
        None,
        description="Actual cost of the appointment",
        ge=0,
        le=Decimal('99999.99')
    )
    ...
    next_appointment_recommended_days: Optional[int] = Field(
        None,
        description="Recommended days until next appointment",
        gt=0,
        le=365
    )
    veterinarian_id: Optional[UUID] = Field(
        None,
        description="UUID of the assigned veterinarian (for reassignment)"
    )
    
    # Use the same validators as AppointmentBase for applicable fields
    _validate_scheduled_at = field_validator('scheduled_at')(AppointmentBase.validate_scheduled_at)
    _validate_service_type_other_description = field_validator('service_type_other_description')(AppointmentBase.validate_service_type_other_description)
    _validate_text_fields = field_validator('reason', 'notes', 'internal_notes', 'follow_up_instructions')(AppointmentBase.validate_text_fields)
    ...
    def validate_cancellation_reason(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if len(v) > 500:
                raise ValueError("Cancellation reason is too long (maximum 500 characters)")
...
class AppointmentResponse(BaseModel):
    ...
    service_type_other_description: Optional[str] = Field(None, description="Service type description for 'other'")
    ...
    follow_up_needed: Optional[bool] = Field(None, description="Whether follow-up is needed")
    follow_up_instructions: Optional[str] = Field(None, description="Follow-up instructions")
    next_appointment_recommended_days: Optional[int] = Field(None, description="Recommended days until next appointment")
...
class AppointmentStatusUpdate(BaseModel):
    ...
    @model_validator(mode='after')
    def validate_cancellation_consistency(self):
    ...
    def validate_reason(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not v:
                return None
            if len(v) > 500:
                raise ValueError("Reschedule reason is too long (maximum 500 characters)")
        return v
...
class AppointmentCompletion(BaseModel):
    ...
    completion_notes: Optional[str] = Field(
        None,
        description="Notes about the completed appointment"
    )
    ...
    def validate_follow_up_consistency(self):
        ...
        if self.follow_up_needed and not self.follow_up_instructions:
            raise ValueError("Follow-up instructions are required when follow-up is needed")
        return self
...
class AppointmentSlotAvailability(BaseModel):
    ...
    veterinarian_id: UUID = Field(..., description="Veterinarian ID to check availability for")
    clinic_id: UUID = Field(..., description="Clinic ID where appointment would take place")
    ...
    duration_minutes: int = Field(30, description="Required appointment duration", gt=0, le=480)
    service_type: Optional[ServiceType] = Field(None, description="Type of service (for specialized availability)")
