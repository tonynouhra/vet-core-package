...
class UserCreate(UserBase):
    ...
    def validate_preferences(cls, v: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        allowed_keys = {
            ...
            'notification_frequency', 'dashboard_layout', 'default_view'
        }
...
class UserUpdate(BaseModel):
    ...
    _validate_phone_number = field_validator('phone_number')(UserBase.validate_phone_number)
    _validate_name_fields = field_validator('first_name', 'last_name')(UserBase.validate_name_fields)
    _validate_postal_code = field_validator('postal_code')(UserBase.validate_postal_code)
    _validate_country = field_validator('country')(UserBase.validate_country)
    _validate_preferences = field_validator('preferences')(UserCreate.validate_preferences)
...
class UserAdminResponse(UserResponse):
    ...
    terms_accepted_at: Optional[str] = Field(None, description="Terms acceptance timestamp")
    privacy_accepted_at: Optional[str] = Field(None, description="Privacy policy acceptance timestamp")
...
class UserStatusUpdate(BaseModel):
    ...
    def validate_status(cls, v: UserStatus) -> UserStatus:
        ...
        if v not in UserStatus:
            raise ValueError(f"Invalid status. Must be one of: {[status.value for status in UserStatus]}")
        return v
...
class PetOwnerCreate(UserCreate):
    ...
    role: UserRole = Field(
        UserRole.PET_OWNER,
        description="Role fixed as pet owner",
        frozen=True
    )
...
class VeterinarianCreate(UserCreate):
    ...
    role: UserRole = Field(
        ...
        description="Role fixed as veterinarian",
        frozen=True
    )
...
class VetTechCreate(UserCreate):
    ...
    role: UserRole = Field(
        UserRole.VET_TECH,
        description="Role fixed as vet tech",
        frozen=True
    )
