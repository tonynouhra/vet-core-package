...
from ..models.pet import PetSpecies, PetGender, PetSize, PetStatus
...
class MedicalRecordSchema(BaseModel):
    ...
    veterinarian: Optional[str] = Field(None, description="Name of attending veterinarian")
...
class AllergySchema(BaseModel):
    ...
    date_discovered: Optional[str] = Field(None, description="When the allergy was discovered")
...
class PetBase(BaseModel):
    ...
    species_other_description: Optional[str] = Field(
        None,
        description="Description when species is 'other'",
        max_length=100
    )
    ...
    mixed_breed: bool = Field(
        False,
        description="Whether the pet is a mixed breed"
    )
    ...
    approximate_age_years: Optional[int] = Field(
        None,
        description="Approximate age in years if birth date unknown",
        ge=0,
        le=50
    )
    approximate_age_months: Optional[int] = Field(
        None,
        description="Approximate age in months if birth date unknown",
        ge=0,
        lt=12
    )
    ...
    size_category: Optional[PetSize] = Field(
        None,
    ...
    spay_neuter_date: Optional[date] = Field(
        None,
        description="Date when pet was spayed or neutered"
    )
    is_microchipped: bool = Field(
        False,
        description="Whether the pet has a microchip"
    )
    microchip_date: Optional[date] = Field(
        None,
        description="Date when microchip was implanted"
    )
    is_insured: bool = Field(
        False,
        description="Whether the pet has insurance"
    )
    insurance_provider: Optional[str] = Field(
        None,
        description="Pet insurance provider name",
        max_length=100
    )
    insurance_policy_number: Optional[str] = Field(
        None,
        description="Pet insurance policy number",
        max_length=100
    )
    temperament: Optional[str] = Field(
        None,
        description="Pet's temperament description",
        max_length=200
    )
    special_needs: Optional[str] = Field(
        None,
        description="Special care needs or requirements"
    )
    behavioral_notes: Optional[str] = Field(
        None,
        description="Behavioral notes and observations"
    )
    profile_photo_url: Optional[str] = Field(
        None,
        description="URL to pet's profile photo",
        max_length=500
    )
    ...
    def validate_name(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9\s\-'.]+$", v):
            raise ValueError("Pet name can only contain letters, numbers, spaces, hyphens, apostrophes, and periods")
    ...
    def validate_species_other_description(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not re.match(r"^[a-zA-Z\s\-']+$", v):
                raise ValueError("Species description can only contain letters, spaces, hyphens, and apostrophes")
    ...
    def validate_breed(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            # Allow letters, numbers, spaces, hyphens, apostrophes, and periods
            if not re.match(r"^[a-zA-Z0-9\s\-'.]+$", v):
                raise ValueError("Breed can only contain letters, numbers, spaces, hyphens, apostrophes, and periods")
    ...
    def validate_microchip_id(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not re.match(r'^[0-9A-Fa-f]+$', v):
                raise ValueError("Microchip ID can only contain numbers and letters A-F")
    ...
    def validate_species_other_consistency(self):
        """Validate that species 'other' has description."""
        if self.species == PetSpecies.OTHER and not self.species_other_description:
            raise ValueError("Species other description is required when species is 'other'")
        return self
    
    @model_validator(mode='after')
    def validate_microchip_consistency(self):
        """Validate microchip consistency."""
        if self.is_microchipped and not self.microchip_id:
            raise ValueError("Microchip ID is required when pet is marked as microchipped")
        return self
    
    @model_validator(mode='after')
    def validate_insurance_consistency(self):
        """Validate insurance consistency."""
        if self.is_insured and not self.insurance_provider:
            raise ValueError("Insurance provider is required when pet is marked as insured")
        return self
    
    @model_validator(mode='after')
    def validate_age_consistency(self):
        """Validate age information consistency."""
        if self.birth_date and (self.approximate_age_years is not None or self.approximate_age_months is not None):
...
class PetCreate(PetBase):
    ...
    owner_id: UUID = Field(
        ...,
        description="UUID of the pet's primary owner"
    )
    ...
    vaccination_records: Optional[List[VaccinationRecordSchema]] = Field(
        None,
        description="Vaccination history and records"
    )
    ...
    preferred_veterinarian_id: Optional[UUID] = Field(
        None,
        description="UUID of preferred veterinarian"
    )
    preferred_clinic_id: Optional[UUID] = Field(
        None,
        description="UUID of preferred clinic"
    )
    
    @field_validator('vaccination_records')
    @classmethod
    def validate_vaccination_records(cls, v: Optional[List[VaccinationRecordSchema]]) -> Optional[List[VaccinationRecordSchema]]:
    ...
    @classmethod
    def validate_allergy_information(cls, v: Optional[List[AllergySchema]]) -> Optional[List[AllergySchema]]:
...
class PetUpdate(BaseModel):
    ...
    species: Optional[PetSpecies] = Field(
        None,
        description="Pet's species"
    )
    species_other_description: Optional[str] = Field(
    ...
    gender: Optional[PetGender] = Field(
        None,
        description="Pet's gender"
    )
    birth_date: Optional[date] = Field(
    ...
    _validate_species_other_description = field_validator('species_other_description')(PetBase.validate_species_other_description)
    ...
    _validate_procedure_dates = field_validator('spay_neuter_date', 'microchip_date')(PetBase.validate_procedure_dates)
    _validate_microchip_id = field_validator('microchip_id')(PetBase.validate_microchip_id)
    _validate_additional_photos = field_validator('additional_photos')(PetBase.validate_additional_photos)
    _validate_vaccination_records = field_validator('vaccination_records')(PetCreate.validate_vaccination_records)
    _validate_allergy_information = field_validator('allergy_information')(PetCreate.validate_allergy_information)
...
class PetResponse(BaseModel):
    ...
    species_other_description: Optional[str] = Field(None, description="Species description for 'other'")
    ...
    approximate_age_years: Optional[int] = Field(None, description="Approximate age in years")
    approximate_age_months: Optional[int] = Field(None, description="Approximate age in months")
    ...
    registration_number: Optional[str] = Field(None, description="Breed registration number")
    status: PetStatus = Field(..., description="Pet's status")
    is_spayed_neutered: bool = Field(..., description="Whether the pet is spayed or neutered")
    spay_neuter_date: Optional[date] = Field(None, description="Date when pet was spayed or neutered")
    is_microchipped: bool = Field(..., description="Whether the pet has a microchip")
    microchip_date: Optional[date] = Field(None, description="Date when microchip was implanted")
    is_insured: bool = Field(..., description="Whether the pet has insurance")
    insurance_provider: Optional[str] = Field(None, description="Pet insurance provider name")
    insurance_policy_number: Optional[str] = Field(None, description="Pet insurance policy number")
    temperament: Optional[str] = Field(None, description="Pet's temperament description")
    special_needs: Optional[str] = Field(None, description="Special care needs or requirements")
    behavioral_notes: Optional[str] = Field(None, description="Behavioral notes and observations")
    profile_photo_url: Optional[str] = Field(None, description="URL to pet's profile photo")
    additional_photos: Optional[List[str]] = Field(None, description="Array of additional photo URLs")
    vaccination_records: Optional[List[VaccinationRecordSchema]] = Field(None, description="Vaccination records")
    allergy_information: Optional[List[AllergySchema]] = Field(None, description="Allergy information")
    emergency_contact: Optional[EmergencyContactSchema] = Field(None, description="Emergency contact")
    preferred_veterinarian_id: Optional[UUID] = Field(None, description="Preferred veterinarian ID")
...
class PetMedicalHistoryUpdate(BaseModel):
    ...
    @classmethod
    def validate_medical_records(cls, v: List[MedicalRecordSchema]) -> List[MedicalRecordSchema]:
...
class PetVaccinationUpdate(BaseModel):
    ...
    @field_validator('vaccination_records')
    @classmethod
    def validate_vaccination_records(cls, v: List[VaccinationRecordSchema]) -> List[VaccinationRecordSchema]:
        """Validate vaccination records."""
        if len(v) == 0:
...
class PetWeightUpdate(BaseModel):
    ...
    weight_kg: Decimal = Field(
        ...,
        description="Pet's new weight in kilograms",
        gt=0,
        le=Decimal('999.99')
    )
    recorded_by: Optional[str] = Field(
        None,
        description="Who recorded the weight"
    )
