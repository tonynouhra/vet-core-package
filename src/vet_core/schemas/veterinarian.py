...
from ..models.veterinarian import VeterinarianStatus, LicenseStatus, EmploymentType
...
class CertificationSchema(BaseModel):
    ...
    expiry_date: Optional[date] = Field(None, description="Expiry date of the certification")
    certification_number: Optional[str] = Field(None, description="Certification number or ID")
    ...
    def validate_certification_number(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not re.match(r'^[A-Z0-9\-]+$', v.upper()):
                raise ValueError("Certification number can only contain letters, numbers, and hyphens")
        
        return v.upper() if v else None
    
    @model_validator(mode='after')
    def validate_expiry_after_obtained(self):
        """Validate expiry date is after obtained date."""
...
class ProfessionalMembershipSchema(BaseModel):
    ...
    organization_name: str = Field(..., description="Name of the professional organization")
    membership_type: Optional[str] = Field(None, description="Type of membership")
    member_since: Optional[date] = Field(None, description="Date when membership started")
    membership_number: Optional[str] = Field(None, description="Membership number or ID")
    is_active: bool = Field(True, description="Whether the membership is currently active")
...
class DayAvailabilitySchema(BaseModel):
    ...
    breaks: Optional[List[AvailabilitySlotSchema]] = Field(None, description="Break periods during the day")
    ...
    @model_validator(mode='after')
    def validate_availability_consistency(self):
...
class VeterinarianBase(BaseModel):
    ...
    license_state: str = Field(
        ...,
        description="State or province where license was issued",
        max_length=100
    )
    license_country: str = Field(
        "US",
        description="Country where license was issued",
        max_length=100
    )
    ...
    veterinary_school: Optional[str] = Field(
        None,
        description="Name of veterinary school attended",
        max_length=200
    )
    graduation_year: Optional[int] = Field(
        None,
        description="Year of graduation from veterinary school",
        ge=1800,
        le=2030
    )
    degree_type: Optional[str] = Field(
        "DVM",
        description="Type of veterinary degree (DVM, VMD, etc.)",
        max_length=50
    )
    years_of_experience: int = Field(
        0,
        description="Years of veterinary experience",
        ge=0,
        le=70
    )
    ...
    specializations: Optional[List[str]] = Field(
        None,
        description="List of medical specializations and areas of expertise"
    )
    services_provided: Optional[List[str]] = Field(
        None,
        description="List of services this veterinarian provides"
    )
    species_expertise: Optional[List[str]] = Field(
        None,
        description="List of animal species this veterinarian specializes in"
    )
    is_accepting_new_patients: bool = Field(
        True,
        description="Whether veterinarian is accepting new patients"
    )
    ...
    professional_interests: Optional[str] = Field(
        None,
        description="Professional interests and research areas"
    )
    languages_spoken: Optional[List[str]] = Field(
        None,
        description="Languages spoken by the veterinarian"
    )
    ...
    def validate_degree_type(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            valid_degrees = {'DVM', 'VMD', 'BVS', 'BVMS', 'BVM&S', 'VET.MED.', 'DR.MED.VET.'}
    ...
    @field_validator('specializations', 'services_provided', 'species_expertise', 'languages_spoken')
    @classmethod
    ...
    @model_validator(mode='after')
    def validate_license_dates_consistency(self):
    ...
    @model_validator(mode='after')
    def validate_experience_consistency(self):
        ...
        if self.graduation_year and self.years_of_experience:
            ...
            if self.years_of_experience > max_possible_experience + 2:  # Allow some flexibility
                raise ValueError("Years of experience seems inconsistent with graduation year")
...
class VeterinarianCreate(VeterinarianBase):
    ...
    user_id: UUID = Field(
        ...,
        description="UUID of the associated user account"
    )
    ...
    status: VeterinarianStatus = Field(
        VeterinarianStatus.ACTIVE,
        description="Initial veterinarian status"
    )
    license_status: LicenseStatus = Field(
        LicenseStatus.ACTIVE,
        description="Initial license status"
    )
    hire_date: Optional[date] = Field(
        None,
        description="Date hired at current clinic"
    )
    additional_certifications: Optional[List[CertificationSchema]] = Field(
        None,
        description="Additional certifications and credentials"
    )
    ...
    @classmethod
    def validate_additional_certifications(cls, v: Optional[List[CertificationSchema]]) -> Optional[List[CertificationSchema]]:
    ...
    @classmethod
    def validate_professional_memberships(cls, v: Optional[List[ProfessionalMembershipSchema]]) -> Optional[List[ProfessionalMembershipSchema]]:
    ...
    @classmethod
    def validate_availability(cls, v: Optional[Dict[str, DayAvailabilitySchema]]) -> Optional[Dict[str, DayAvailabilitySchema]]:
        ...
        for day, schedule in v.items():
            ...
            if not isinstance(schedule, DayAvailabilitySchema):
                raise ValueError(f"Availability for {day} must be a DayAvailabilitySchema")
...
class VeterinarianUpdate(BaseModel):
    ...
    status: Optional[VeterinarianStatus] = Field(
        None,
        description="Veterinarian status"
    )
    ...
    clinic_id: Optional[UUID] = Field(
        None,
        description="UUID of the primary clinic (for transfers)"
    )
    
    # Use the same validators as VeterinarianBase for applicable fields
    _validate_license_number = field_validator('license_number')(VeterinarianBase.validate_license_number)
    _validate_location_fields = field_validator('license_state', 'license_country')(VeterinarianBase.validate_location_fields)
    _validate_license_dates = field_validator('license_issued_date', 'license_expiry_date')(VeterinarianBase.validate_license_dates)
    _validate_veterinary_school = field_validator('veterinary_school')(VeterinarianBase.validate_veterinary_school)
    _validate_degree_type = field_validator('degree_type')(VeterinarianBase.validate_degree_type)
    _validate_graduation_year = field_validator('graduation_year')(VeterinarianBase.validate_graduation_year)
    _validate_string_lists = field_validator('specializations', 'services_provided', 'species_expertise', 'languages_spoken')(VeterinarianBase.validate_string_lists)
    _validate_emergency_contact_number = field_validator('emergency_contact_number')(VeterinarianBase.validate_emergency_contact_number)
    _validate_text_fields = field_validator('bio', 'professional_interests')(VeterinarianBase.validate_text_fields)
    _validate_hire_date = field_validator('hire_date')(VeterinarianCreate.validate_hire_date)
    _validate_additional_certifications = field_validator('additional_certifications')(VeterinarianCreate.validate_additional_certifications)
    _validate_professional_memberships = field_validator('professional_memberships')(VeterinarianCreate.validate_professional_memberships)
    _validate_availability = field_validator('availability')(VeterinarianCreate.validate_availability)
...
class VeterinarianResponse(BaseModel):
    ...
    veterinary_school: Optional[str] = Field(None, description="Veterinary school attended")
    ...
    services_provided: Optional[List[str]] = Field(None, description="Services provided")
    species_expertise: Optional[List[str]] = Field(None, description="Species expertise")
    is_accepting_new_patients: bool = Field(..., description="Accepting new patients")
    appointment_duration_minutes: Optional[int] = Field(None, description="Default appointment duration")
    max_daily_appointments: Optional[int] = Field(None, description="Maximum daily appointments")
    ...
    professional_interests: Optional[str] = Field(None, description="Professional interests")
    ...
    additional_certifications: Optional[List[Dict[str, Any]]] = Field(None, description="Additional certifications")
    professional_memberships: Optional[List[Dict[str, Any]]] = Field(None, description="Professional memberships")
...
class VeterinarianStatusUpdate(BaseModel):
    ...
    def validate_status(cls, v: VeterinarianStatus) -> VeterinarianStatus:
        ...
        if v not in VeterinarianStatus:
            raise ValueError(f"Invalid status. Must be one of: {[status.value for status in VeterinarianStatus]}")
        return v
...
class VeterinarianLicenseUpdate(BaseModel):
    ...
    license_status: Optional[LicenseStatus] = Field(None, description="New license status")
    license_issued_date: Optional[date] = Field(None, description="New license issued date")
    license_expiry_date: Optional[date] = Field(None, description="New license expiry date")
...
class VeterinarianAvailabilityUpdate(BaseModel):
    ...
    @field_validator('availability')
    @classmethod
    def validate_availability(cls, v: Dict[str, DayAvailabilitySchema]) -> Dict[str, DayAvailabilitySchema]:
        """Validate availability schedule."""
        valid_days = {'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'}
...
class VeterinarianSearchFilters(BaseModel):
    ...
    specializations: Optional[List[str]] = Field(None, description="Filter by specializations")
    services_provided: Optional[List[str]] = Field(None, description="Filter by services")
    species_expertise: Optional[List[str]] = Field(None, description="Filter by species expertise")
    is_accepting_new_patients: Optional[bool] = Field(None, description="Filter by patient acceptance")
    min_rating: Optional[float] = Field(None, description="Minimum rating filter", ge=0, le=5)
    min_experience_years: Optional[int] = Field(None, description="Minimum years of experience", ge=0)
    employment_type: Optional[EmploymentType] = Field(None, description="Filter by employment type")
    languages_spoken: Optional[List[str]] = Field(None, description="Filter by languages spoken")
...
class VeterinarianRatingUpdate(BaseModel):
    ...
    new_rating: Decimal = Field(
        ...,
        description="New rating to add (0.0 to 5.0)",
        ge=0,
        le=5
    )
