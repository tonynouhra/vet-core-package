...
class DocsWorkflowSubtasksTest:
    ...
    def test_workflow_structure_v4(self):
        ...
        try:
                ...
                for step in steps:
                    if isinstance(step, dict) and "uses" in step:
                        ...
                        if "actions/upload-artifact@v4" in uses:
                            ...
                            "actions/upload-artifact@v3" in uses
                            or "actions/download-artifact@v3" in uses
                        ):
            ...
            if v3_actions_found:
                self.log(
                    "❌ Found v3 artifact actions that should be upgraded:", "ERROR"
                )
            ...
            if v4_upload_found and v4_download_found:
                ...
                self.log(
                    "⚠️ Some v4 actions found, but not all expected ones", "WARNING"
                )
                self.test_results["workflow_structure_v4"] = True
                return True
            else:
    ...
    def test_artifact_upload_config(self):
        ...
        try:
            ...
            if len(found_patterns) >= 2:  # At least v4 and some artifact config
                ...
                self.log(
                    "❌ Documentation artifact upload configuration incomplete", "ERROR"
                )
                return False
    ...
    def test_pages_deployment_config(self):
        ...
        try:
            ...
            if len(found_patterns) >= 4:  # Most essential patterns found
                ...
                self.log(
                    f"⚠️ GitHub Pages deployment configuration may be incomplete ({len(found_patterns)}/6 patterns found)",
                    "WARNING",
                )
    ...
    def test_api_coverage_config(self):
        ...
        try:
            ...
            if len(found_patterns) >= 2 or len(existing_files) >= 1:
                ...
                self.log(
                    "❌ API documentation coverage configuration incomplete", "ERROR"
                )
                return False
    ...
    def test_documentation_structure(self):
        ...
        essential_files = ["conf.py", "index.rst", "Makefile"]
        ...
        additional_files = ["API_REFERENCE.md", "USAGE_GUIDE.md"]
    ...
    def generate_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            if self.test_results["overall_success"]:
                f.write(
                    "🎉 **TEST SUCCESSFUL** - Documentation workflow subtasks are properly configured.\n\n"
                )
            else:
            ...
            f.write(
                "1. **Workflow Structure (v4)**: Verified that the workflow uses actions/upload-artifact@v4 and actions/download-artifact@v4\n"
            )
            f.write(
                "2. **Artifact Upload Config**: Checked documentation artifact upload configuration\n"
            )
            f.write(
                "3. **Pages Deployment Config**: Validated GitHub Pages deployment setup with v4 artifacts\n"
            )
            f.write(
                "4. **API Coverage Config**: Tested API documentation coverage artifact configuration\n"
            )
            f.write(
                "5. **Documentation Structure**: Verified essential documentation files and structure\n\n"
            )
            f.write("## Requirements Addressed\n\n")
            f.write(
                "- **1.1**: GitHub Actions workflows use supported artifact actions (v4)\n"
            )
