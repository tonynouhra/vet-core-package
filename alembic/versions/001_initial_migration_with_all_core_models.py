...
def upgrade() -> None:
    # Create enum types
    op.execute(
        "CREATE TYPE userrole AS ENUM ('pet_owner', 'veterinarian', 'vet_tech', 'clinic_admin', 'platform_admin')"
    )
    op.execute(
        "CREATE TYPE userstatus AS ENUM ('active', 'inactive', 'suspended', 'pending_verification')"
    )
    op.execute(
        "CREATE TYPE petspecies AS ENUM ('dog', 'cat', 'bird', 'rabbit', 'hamster', 'guinea_pig', 'ferret', 'reptile', 'fish', 'other')"
    )
    op.execute("CREATE TYPE petgender AS ENUM ('male', 'female', 'unknown')")
    op.execute(
        "CREATE TYPE petsize AS ENUM ('extra_small', 'small', 'medium', 'large', 'extra_large')"
    )
    op.execute(
        "CREATE TYPE petstatus AS ENUM ('active', 'inactive', 'deceased', 'lost', 'transferred')"
    )
    op.execute(
        "CREATE TYPE appointmentstatus AS ENUM ('scheduled', 'confirmed', 'checked_in', 'in_progress', 'completed', 'cancelled', 'no_show', 'rescheduled')"
    )
    op.execute(
        "CREATE TYPE servicetype AS ENUM ('wellness_exam', 'vaccination', 'dental_cleaning', 'surgery', 'emergency', 'grooming', 'boarding', 'consultation', 'follow_up', 'diagnostic', 'treatment', 'other')"
    )
    op.execute(
        "CREATE TYPE appointmentpriority AS ENUM ('low', 'normal', 'high', 'urgent', 'emergency')"
    )
    op.execute(
        "CREATE TYPE clinicstatus AS ENUM ('active', 'inactive', 'temporarily_closed', 'permanently_closed', 'under_renovation')"
    )
    op.execute(
        "CREATE TYPE clinictype AS ENUM ('general_practice', 'emergency', 'specialty', 'mobile', 'hospital', 'urgent_care')"
    )
    op.execute(
        "CREATE TYPE veterinarianstatus AS ENUM ('active', 'inactive', 'on_leave', 'suspended', 'retired')"
    )
    op.execute(
        "CREATE TYPE licensestatus AS ENUM ('active', 'expired', 'suspended', 'revoked', 'pending_renewal')"
    )
    op.execute(
        "CREATE TYPE employmenttype AS ENUM ('full_time', 'part_time', 'contract', 'locum', 'owner', 'partner')"
    )
    ...
    op.create_index(
        "idx_users_preferences_gin", "users", ["preferences"], postgresql_using="gin"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "services_offered", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "specialties", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.Column(
            "facility_features", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "equipment_available",
        ...
        sa.Column(
            "insurance_accepted", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
    ...
    op.create_index(
        "idx_clinics_accepts_new_patients",
        "clinics",
        ["accepts_new_patients", "status"],
    )
    op.create_index(
        "idx_clinics_accepts_emergencies", "clinics", ["accepts_emergencies", "status"]
    )
    op.create_index(
        "idx_clinics_accepts_walk_ins", "clinics", ["accepts_walk_ins", "status"]
    )
    op.create_index(
    ...
    op.create_index(
        "idx_clinics_operating_hours_gin",
        "clinics",
        ["operating_hours"],
    ...
    op.create_table(
        ...
        sa.Column(
            "services_provided", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "species_expertise", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "availability", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.Column(
            "languages_spoken", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("emergency_contact_number", sa.String(length=20), nullable=True),
        sa.Column(
    ...
    op.create_index(
        "idx_veterinarians_license_number", "veterinarians", ["license_number"]
    )
    op.create_index("idx_veterinarians_status", "veterinarians", ["status"])
    op.create_index(
        "idx_veterinarians_license_status", "veterinarians", ["license_status"]
    )
    op.create_index(
        "idx_veterinarians_employment_type", "veterinarians", ["employment_type"]
    )
    op.create_index(
        "idx_veterinarians_is_accepting_new_patients",
        "veterinarians",
        ["is_accepting_new_patients"],
    )
    op.create_index("idx_veterinarians_rating", "veterinarians", ["rating"])
    op.create_index(
        "idx_veterinarians_license_expiry_date",
        "veterinarians",
        ["license_expiry_date"],
    )
    op.create_index(
        "idx_veterinarians_clinic_status", "veterinarians", ["clinic_id", "status"]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "gender",
            sa.Enum("male", "female", "unknown", name="petgender"),
            nullable=False,
        ),
        ...
        sa.Column(
            "size_category",
            sa.Enum(
                "extra_small", "small", "medium", "large", "extra_large", name="petsize"
            ),
            nullable=True,
        ),
        ...
        sa.Column(
            "additional_photos", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
        ...
        sa.Column(
            "medication_history", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "allergy_information",
            postgresql.JSONB(astext_type=sa.Text()),
        ...
        sa.Column(
            "preferred_veterinarian_id", postgresql.UUID(as_uuid=True), nullable=True
        ),
        ...
        sa.ForeignKeyConstraint(
            ["preferred_clinic_id"], ["clinics.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["preferred_veterinarian_id"], ["veterinarians.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        "idx_pets_preferred_veterinarian_id", "pets", ["preferred_veterinarian_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "service_type_other_description", sa.String(length=200), nullable=True
        ),
        sa.Column(
            "status",
            sa.Enum(
        ...
        sa.CheckConstraint(
            "scheduled_at > created_at", name="ck_appointments_scheduled_after_created"
        ),
        sa.CheckConstraint(
        ...
        sa.ForeignKeyConstraint(
            ["veterinarian_id"], ["veterinarians.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "idx_appointments_veterinarian_id", "appointments", ["veterinarian_id"]
    )
    ...
    op.create_index(
        "idx_appointments_pet_scheduled", "appointments", ["pet_id", "scheduled_at"]
    )
    op.create_index(
        "idx_appointments_vet_scheduled",
        "appointments",
        ["veterinarian_id", "scheduled_at"],
    )
    op.create_index(
        "idx_appointments_clinic_scheduled",
        "appointments",
        ["clinic_id", "scheduled_at"],
    )
    op.create_index(
        "idx_appointments_status_scheduled", "appointments", ["status", "scheduled_at"]
    )
