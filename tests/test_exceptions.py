...
class TestDatabaseException:
    ...
    def test_connection_exception_creation(self):
        ...
        exc = ConnectionException("Connection failed", database_url="postgresql://user:pass@localhost/db")
...
class TestValidationException:
    ...
    def test_validation_exception_creation(self):
        ...
        exc = ValidationException("Validation failed", field="email", value="invalid-email")
...
class TestExceptionUtilities:
    ...
    def test_config_value_sanitization_edge_cases(self):
        ...
        for key in sensitive_keys:
            exc = ConfigurationException(config_key=f"test_{key}", config_value="sensitive")
            assert exc.details["config_value"] == "[REDACTED]"
...
class TestUtilityFunctions:
    ...
    def test_format_validation_errors(self):
        ...
        pydantic_errors = [
            ...
            {"loc": ["name", "first"], "msg": "str type expected", "type": "type_error"},
        ]
...
class TestDatabaseRetryDecorator:
    ...
    async def test_retryable_database_exception(self):
        ...
        async def failing_operation():
            ...
            call_count += 1
            if call_count < 3:
                raise DatabaseException("Connection failed", retry_count=call_count-1, max_retries=2)
            return "success"
    ...
    async def test_non_retryable_database_exception(self):
        ...
        @handle_database_retry("test_operation", max_retries=2)
        async def failing_operation():
            raise DatabaseException("Auth failed", retry_count=3, max_retries=2)  # Not retryable
