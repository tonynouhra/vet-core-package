...
class TestUserModel:
    ...
    def test_user_profile_update(self):
        ...
        user.update_profile(
            first_name="Jane", phone_number="+1234567890", city="New York"
        )
    ...
    def test_user_repr(self):
        ...
        expected = (
            f"<User(id={user.id}, email='test@example.com', role='veterinarian')>"
        )
        assert repr(user) == expected

    @pytest_asyncio.fixture
    async def test_user_pet_relationship(
    ...
    @pytest_asyncio.fixture
    async def test_user_veterinarian_relationship(
        self, async_session, user_factory, veterinarian_factory, clinic_factory
    ):
        ...
        veterinarian = await veterinarian_factory.create(
            async_session, user=vet_user, clinic=clinic
        )
    ...
    def test_user_preferences_edge_cases(self):
        ...
        assert user.get_preference("settings") == {
            "notifications": True,
            "language": "en",
        }
    ...
    def test_user_profile_update_edge_cases(self):
        ...
        with pytest.raises(ValueError, match="Invalid profile field"):
            ...
            )  # Email not allowed in profile update
