...
from vet_core.models import (
    ...
    UserRole, PetSpecies, AppointmentStatus
)
...
class DatabaseTestUtils:
    ...
    async def get_record_by_id(self, model_class: Type[T], record_id: uuid.UUID) -> Optional[T]:
        """Get a record by ID."""
        query = select(model_class).where(model_class.id == record_id)
        result = await self.session.execute(query)
...
class TestDataBuilder:
    ...
    async def create_user_with_pets(
        ...
        from .conftest import UserFactory, PetFactory
        ...
        for i in range(pet_count):
            pet_data = pet_data_list[i] if pet_data_list and i < len(pet_data_list) else {}
    ...
    async def create_clinic_with_veterinarians(
        ...
        from .conftest import ClinicFactory, VeterinarianFactory, UserFactory
        ...
        for i in range(vet_count):
            vet_data = vet_data_list[i] if vet_data_list and i < len(vet_data_list) else {}
...
class AssertionHelpers:
    ...
    def assert_user_fields(user: User, expected_data: Dict[str, Any]):
        ...
        for field, expected_value in expected_data.items():
            ...
            assert actual_value == expected_value, f"User.{field}: expected {expected_value}, got {actual_value}"
    ...
    def assert_pet_fields(pet: Pet, expected_data: Dict[str, Any]):
        ...
        for field, expected_value in expected_data.items():
            ...
            assert actual_value == expected_value, f"Pet.{field}: expected {expected_value}, got {actual_value}"
    
    @staticmethod
    def assert_appointment_fields(appointment: Appointment, expected_data: Dict[str, Any]):
        """Assert that appointment fields match expected data."""
        for field, expected_value in expected_data.items():
            actual_value = getattr(appointment, field)
            assert actual_value == expected_value, f"Appointment.{field}: expected {expected_value}, got {actual_value}"
    
    @staticmethod
    def assert_record_count(actual_count: int, expected_count: int, entity_name: str = "records"):
        """Assert record count with descriptive message."""
        assert actual_count == expected_count, f"Expected {expected_count} {entity_name}, got {actual_count}"
    ...
    @staticmethod
    def assert_record_not_exists(record: Optional[BaseModel], entity_name: str = "record"):
    ...
    def assert_decimal_close(actual: Decimal, expected: Decimal, tolerance: Decimal = Decimal('0.01')):
        ...
        assert diff <= tolerance, f"Decimal difference {diff} exceeds tolerance {tolerance}"
