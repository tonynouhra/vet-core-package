...
class TestComplianceIntegration:
    ...
    def test_complete_audit_trail_workflow(
        ...
        violations, metrics = compliance_manager.check_compliance(sample_security_report)
        ...
        # Verify event counts
        assert len([e for e in all_events if e.event_type == AuditEventType.VULNERABILITY_DETECTED]) == 3
        assert len([e for e in all_events if e.event_type == AuditEventType.VULNERABILITY_RESOLVED]) == 2
    ...
    def test_iso27001_compliance_reporting(
        ...
        expected_controls = ["A.12.6.1", "A.16.1.3"]  # Technical vulnerabilities, reporting
    ...
    def test_policy_violation_detection_and_tracking(
        ...
        critical_violations = [
            ...
            if v.vulnerability_id == "CRITICAL-001" and v.violation_type == "resolution_time_exceeded"
        ]
        ...
        high_violations = [
            v for v in violations 
            if v.vulnerability_id == "HIGH-001" and v.violation_type == "resolution_time_exceeded"
        ]
        ...
        medium_violations = [
            v for v in violations 
            if v.vulnerability_id == "MEDIUM-001"
        ]
