...
from vet_core.models import AppointmentStatus, PetSpecies, ServiceType, UserRole
...
async def create_clinic_example():
    ...
    try:
        ...
        async with get_transaction() as session:
            ...
            print(
                f"✓ Clinic created: {clinic_response.name} (ID: {clinic_response.id})"
            )
...
async def create_veterinarian_example(user_id: str, clinic_id: str):
    ...
    try:
        ...
        async with get_transaction() as session:
            ...
            print(
                f"✓ Veterinarian created: {vet_response.license_number} (ID: {vet_response.id})"
            )
...
async def create_appointment_example(pet_id: str, veterinarian_id: str, clinic_id: str):
    ...
    try:
        ...
        async with get_transaction() as session:
            ...
            print(
                f"✓ Appointment scheduled: {appointment_response.scheduled_at} (ID: {appointment_response.id})"
            )
    ...
    try:
        ...
        if pet and clinic:
            ...
            async with get_transaction() as session:
                ...
                veterinarian = await create_veterinarian_example(
                    str(vet_user.id), clinic.id
                )
