...
from vet_core.exceptions import BusinessRuleException, handle_database_retry
    ...
    async with get_session() as session:
        try:
            ...
            for vet, total, avg_cost, last_appt in vet_stats:
                print(
                    f"  - Dr. {vet.user.last_name}: {total or 0} appointments, "
            ...
            stmt = (
                ...
                .options(selectinload(Pet.owner), selectinload(Pet.appointments))
                .distinct()
            )
            ...
            print(
                f"✓ Found {len(pets_with_appointments)} pets with upcoming appointments"
            )
            ...
            stmt = (
                select(
                    ...
                    func.avg(Appointment.estimated_duration_minutes).label(
                        "avg_duration"
                    ),
                )
                .join(Appointment)
    ...
    def validate_pet_age(birth_date: date) -> None:
        ...
        if age_years > 30:  # Very old for most pets
            raise ValidationException(
                ...
                value=birth_date,
                validation_errors={
                    "birth_date": [f"Age calculated as {age_years:.1f} years"]
                },
            )
    ...
    async def create_pet_with_retry(pet_data: dict, should_fail: bool = False):
        ...
        if should_fail:
            raise DatabaseException(
                "Simulated transient database error", retry_count=0, max_retries=3
            )
...
async def performance_optimization_example():
    ...
    async with get_session() as session:
        try:
            ...
            print(
                f"✓ Loaded {len(owners)} owners with {total_pets} pets using batch loading"
            )
            ...
            if stats:
                print(
                    f"✓ Pet statistics: {stats.total_pets} total, "
...
async def testing_patterns_example():
    ...
    async with get_transaction() as session:
        try:
            ...
            test_user = UserFactory.create(first_name="Factory", last_name="User")
            session.add(test_user)
            await session.flush()

            test_pet = PetFactory.create(
                owner_id=test_user.id, name="Factory Pet", species=PetSpecies.CAT
            )
            ...
            print(
                f"✓ Created test pet: {test_pet.name} owned by {test_user.first_name}"
            )
